...
This attribute is used in the <tt>drush::default</tt> recipe only.

Specifies whether to install Drush using PEAR (+pear+) or by cloning from source (+git+).

Defaults to +pear+.

=== install_dir

This attribute is used in the <tt>drush::git</tt> recipe.

Specifies where Drush should be installed to.

Defaults to <tt>/usr/share/php/drush</tt>.
...
This attribute is used in the in the <tt>drush::pear</tt> recipe.

Specifies where to get a list of available Drush releases.

Defaults to <tt>http://pear.drush.org/rest/r/drush/allreleases.xml</tt>.
